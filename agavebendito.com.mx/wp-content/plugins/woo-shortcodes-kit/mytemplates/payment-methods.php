...
// ADD YOUR CONTAINERS ID
        ...
        $wshkmipaymentsid = get_option('wshk_mipaymentsid');
...
<?php if ( $has_methods ) : ?>
	...
	<table class="woocommerce-MyAccount-paymentMethods shop_table shop_table_responsive account-payment-methods-table">
		...
		<?php foreach ( $saved_methods as $type => $methods ) : ?>
			...
			<?php $wshkmipaymentsid = get_option('wshk_mipaymentsid'); ?>
						...
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							if ( has_action( 'woocommerce_account_payment_methods_column_' . $column_id ) ) {
								...
								foreach ( $method['actions'] as $key => $action ) {
									echo '<a href="' . esc_url( $action['url'] ) . $wshkmipaymentsid .'" class="button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>&nbsp;';
								}
							}
							?>
						</td>
...
<?php if ( WC()->payment_gateways->get_available_payment_gateways() ) : ?>
	<a class="button" id="metopagoboton" href="<?php echo esc_url( wc_get_endpoint_url( 'add-payment-method' ) ); ?><?php echo $wshkmipaymentsid; ?>"><?php esc_html_e( 'Add payment method', 'woocommerce' ); ?></a>
<?php endif; ?>
