							...
							if ( has_action( 'woocommerce_account_downloads_column_' . $column_id ) ) {
								...
								switch ( $column_id ) {
									case 'download-product' : ?>
										...
										<?php break;
									case 'download-file' : ?>
										<a href="<?php echo esc_url( $download['download_url'] ); ?>" class="woocommerce-Button button"><!--<span class="dashicons dashicons-download" style="margin-top: 10px;"></span>--> <?php esc_html_e( 'Download', 'woocommerce' ); ?>
										...
										<?php
											...
											if ( $actions = apply_filters( 'woocommerce_account_download_actions', $actions, $download ) ) {
												foreach ( $actions as $key => $action ) {
													echo '<a href="' . esc_url( $action['url'] ) . '" style="woocommerce-Button button' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
												}
											}
										?>
										<?php break;
