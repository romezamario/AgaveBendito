...
(() => {
    ...
    (() => {
        ...
        var AgeGateHooks = window.AgeGateHooks, age_gate_submit = function() {
            var _ref = _asyncToGenerator(_regeneratorRuntime().mark((function _callee2(form, submitter) {
                var formData, asString, response, data;
                return _regeneratorRuntime().wrap((function(_context2) {
                    for (;;) switch (_context2.prev = _context2.next) {
                      case 0:
                        return formData = new FormData(form), asString = new URLSearchParams(formData).toString(), 
                        _context2.next = 4, axios_default().get("".concat(uri, "?").concat(asString)).catch(function() {
                            var _ref2 = _asyncToGenerator(_regeneratorRuntime().mark((function _callee(err) {
                                var message;
                                return _regeneratorRuntime().wrap((function(_context) {
                                    for (;;) switch (_context.prev = _context.next) {
                                      case 0:
                                        if (message = err.message, formData.append("action", "ag_check"), !fallback) {
                                            _context.next = 8;
                                            break;
                                        }
                                        return _context.next = 5, axios_default().post(fallback, formData).catch((function(err) {
                      ...
                      case 4:
                        return response = _context2.sent, data = response.data, _context2.abrupt("return", data);
        ...
        }(), munged = document.querySelector("[data-ag-munge]");
        munged && (__webpack_require__.g.age_gate = JSON.parse(window.atob(munged.dataset.agMunge)));
        var _window2$age_gate = window.age_gate, cookieDomain = _window2$age_gate.cookieDomain, cookieName = _window2$age_gate.cookieName, age = _window2$age_gate.age, userAgents = _window2$age_gate.userAgents, customTitle = _window2$age_gate.customTitle, viewport = _window2$age_gate.viewport, rechallenge = _window2$age_gate.rechallenge, rechallengeError = _window2$age_gate.error, apiError = _window2$age_gate.generic, uri = _window2$age_gate.uri, fallback = _window2$age_gate.fallback, useLocalStorage = _window2$age_gate.useLocalStorage, getStore = function(key) {
            if (useLocalStorage) {
                var data = localStorage.getItem(key);
                if (data) try {
                    var item = JSON.parse(data), value = item.value, expires = item.expires;
                    return (new Date).getTime() > expires && (localStorage.removeItem(key), function(r) {
                        throw new TypeError('"' + r + '" is read-only');
                    }("value")), value;
                } catch (error) {
                    return localStorage.removeItem(key), !1;
                }
                return !1;
            }
            return parseInt(api.get(key));
        }, setStore = function(key, value, cookieOptions) {
            if (useLocalStorage) {
                var item = {
                    value,
            ...
            } else api.set(key, value, cookieOptions);
        }, cookieOptions = {
            domain: cookieDomain,
            path: "/",
        ...
        if (customTitle && (window.addEventListener("age_gate_shown", (function() {
            var _document$querySelect, safeTitle = null;
            munged && (safeTitle = null === (_document$querySelect = document.querySelector(".age-gate").dataset) || void 0 === _document$querySelect ? void 0 : _document$querySelect.title);
            document.title = safeTitle || customTitle;
        })), window.addEventListener("age_gate_passed", (function() {
            document.title = pageTitle;
        }))), viewport) {
            var metaTag = document.querySelector('meta[name="viewport"]'), freshTag = document.createElement("meta"), metaValue = null;
            metaTag && (metaValue = metaTag.content), window.addEventListener("age_gate_shown", (function() {
                metaTag ? metaTag.content = "width=device-width, minimum-scale=1, maximum-scale=1" : (freshTag.content = "width=device-width, minimum-scale=1, maximum-scale=1", 
                document.head.appendChild(freshTag));
            })), window.addEventListener("age_gate_passed", (function() {
                metaTag ? metaTag.content = metaValue : document.head.removeChild(freshTag);
            }));
        }
        var noRechallenge = function(event) {
            var type = event.type;
            if ("" === rechallenge && getStore("".concat(cookieName, "_failed")) || !1 === rechallenge && getStore("".concat(cookieName, "_failed")) || "" === rechallenge && "age_gate_failed" === type || !1 === rechallenge && "age_gate_failed" === type) {
                Array.from(document.querySelectorAll(".age-gate__fields, .age-gate-fields, .age-gate__remember-wrapper, .age-gate-remember-wrapper, .age-gate__extra, .age-gate-extra, .age-gate__submit, .age-gate-submit")).forEach((function(el) {
                    return el.parentNode.removeChild(el);
                }));
                var p = document.createElement("p");
                p.innerHTML = rechallengeError, p.className = "age-gate__error", document.querySelector(".age-gate__errors, .age-gate-errors").appendChild(p);
            }
        };
        window.addEventListener("age_gate_shown", noRechallenge), window.addEventListener("age_gate_failed", noRechallenge);
        var bot = function() {
            return !!(new src.Crawler).isCrawler(navigator.userAgent) || -1 !== userAgents.indexOf(navigator.userAgent);
        }, _show = function() {
            var form, force = arguments.length > 0 && void 0 !== arguments[0] && arguments[0];
            if (document.body) {
                var ag_logged_in = window.ag_logged_in;
                if (force || (getStore(cookieName) || 0) < age && !bot() && !ag_logged_in) {
                    var template = document.getElementById("tmpl-age-gate").innerHTML;
                    document.body.insertAdjacentHTML("afterbegin", template), document.body.classList.add("age-restriction"), 
                    document.body.parentElement.classList.add("age-gate__restricted"), document.body.parentElement.classList.add("age-gate__restricted--js"), 
                    fireEvent("age_gate_shown"), fireEvent("agegateshown"), function(form) {
                        form.querySelectorAll("button").forEach((function(btn) {
                            btn.addEventListener("click", (function(e) {
                                return form.querySelector('input[name="age_gate[confirm]"]').value = e.currentTarget.value;
                            }));
                        }));
                    }(form = document.querySelector(".age-gate__form, .age-gate-form")), form.addEventListener("submit", function() {
                        var _ref3 = _asyncToGenerator(_regeneratorRuntime().mark((function _callee3(e) {
                            var data, status, redirect, errors, cookieLength, transition, set_cookie, user_age, url, current, now, destination, messages;
                            return _regeneratorRuntime().wrap((function(_context3) {
                                for (;;) switch (_context3.prev = _context3.next) {
                                  case 0:
                                    return e.preventDefault(), document.body.classList.add("age-restriction--working"), 
                                    _context3.next = 5, age_gate_submit(e.target, e.submitter);

                                  case 5:
                                    data = _context3.sent, status = data.status, redirect = data.redirect, errors = data.errors, 
                                    cookieLength = data.cookieLength, transition = data.transition, set_cookie = data.set_cookie, 
                                    cookieLength && Object.assign(cookieOptions, {
                                        expires: cookieLength
                                    }), !0 === status ? (user_age = data.data.user_age, set_cookie && (setStore(cookieName, user_age, cookieOptions), 
                                    key = "".concat(cookieName, "_failed"), useLocalStorage ? localStorage.removeItem(key) : api.remove(key)), 
                                    fireEvent("age_gate_passed", data), fireEvent("agegatepassed", data), redirect && (url = new URL(redirect), 
                                    current = new URL(document.URL), now = "".concat(current.host).concat(current.pathname), 
                                    destination = "".concat(url.host).concat(url.pathname), now.replace(/\/$/, "") != destination.replace(/\/$/, "") && (window.location.href = redirect)), 
                                    hide(transition)) : (fireEvent("age_gate_failed"), fireEvent("agegatefailed"), messages = '<p class="age-gate__error">'.concat(errors[Object.keys(errors).pop()], "</p>"), 
                                    document.querySelector(".age-gate__errors, .age-gate-errors").innerHTML = messages, 
                                    set_cookie && setStore("".concat(cookieName, "_failed"), 1, cookieOptions), redirect && (window.location.href = redirect)), 
            ...
            } else setTimeout(_show, 1);
        }, hide = function() {
            var transition = arguments.length > 0 && void 0 !== arguments[0] && arguments[0], ageGate = document.querySelector(".age-gate__wrapper, .age-gate-wrapper");
            transition && [ "slide-up", "slide-down", "slide-left", "slide-right", "fade" ].indexOf(transition) > -1 ? (ageGate.addEventListener("transitionend", (function() {
                document.body.classList.remove("age-restriction"), document.body.parentElement.classList.remove("age-gate__restricted"), 
                document.body.parentElement.classList.remove("age-gate__restricted--js"), ageGate.parentNode.removeChild(ageGate), 
                fireEvent("age_gate_hidden"), fireEvent("agegatehidden");
            })), ageGate.classList.add("age-gate--".concat(transition))) : (document.body.classList.remove("age-restriction"), 
            document.body.parentElement.classList.remove("age-gate__restricted"), document.body.parentElement.classList.remove("age-gate__restricted--js"), 
            ageGate.parentNode.removeChild(ageGate), fireEvent("age_gate_hidden"), fireEvent("agegatehidden"));
        };
        if (AgeGateHooks) {
            var ag_logged_in = window.ag_logged_in;
            AgeGateHooks.applyFilters("age_gate_show", (getStore(cookieName) || 0) < age && !bot() && !ag_logged_in) && setTimeout((function() {
                return _show(!0);
            }), AgeGateHooks.applyFilters("age_gate_show_timeout", 1));
        } else _show();
        __webpack_require__.g.age_gate_show = _show, __webpack_require__.g.age_gate_hide = hide;
    })();
})();